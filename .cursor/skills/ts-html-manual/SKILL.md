...
- VFD font only on complete display strings: `.vfd-screen` cells and `span.lcd` (`DELAY=0000`, `WORKING...`). Parameter names in body text and headings use ordinary type. Lone starred LCD actions (`*YES*`, `*EXIT*`) are `span.function` with `span.star`, not `lcd`.
- Tag `dfn.term` (first mention of a term string in an `h4` stretch, or `h3` if that stretch has no `h4`; later mentions are `span.term`), `span.param`, `span.value`, `span.lcd`, `span.file-type` (disk types from the Storage `file-types` table), `span.function` (including LCD actions `*YES*` / `*EXIT*` / `*LOAD*` with `span.star` Anybody Star `*`; leave VFD asterisks), `kbd.button`, `span.key-group`, `aside.tip` / `aside.model-note`. File *names* stay `span.patch-name`. Panel names from `notes/terms.json` become `kbd.button` next to press/hold/click/double-click or the word *button*. Silk-screen **group** legends (`Track Parameters`, `Programming`, `Seq/Song Tracks`, BankSet legends) are `span.key-group` only when they help locate a control — not when the words name an architecture object. Split `Seq/Song Tracks` from the `1-6` / `7-12` buttons. On VFD mockups, `.selected` is a solid underline (primary/cursor); `.flash-underline` is a broken underline (layered track) painted with a repeating gradient, flashing on screen.
